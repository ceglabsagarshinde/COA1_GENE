...
p[[i]]=ggplot(c, aes(x = nBg, y = Tp)) + labs(caption = paste(mdl," nucleotide frequency model", sep="")) + ggtitle("Pseudogenization time estimation using single substitution rate") + geom_line(aes(color = Foreground)) + scale_color_brewer(palette = "Dark2") + scale_x_continuous(breaks=seq(0, 40, 5),name ="Number of Background species (nBg)")+ scale_y_continuous(breaks=seq(0, 21, 3),name ="Time of pseudogenization (MYA)")
theme_set( theme_minimal() + theme(plot.title = element_text(hjust = 0.5),text = element_text(size=15),legend.position = "right",axis.line = element_line(color = "black",size = 0.7, linetype = "solid")))
p[[i+1]]=ggplot(c, aes(x = nBg, y = Tp2)) + labs(caption = paste(mdl," nucleotide frequency model", sep="")) + ggtitle("Pseudogenization time estimation using different substitution rate") + geom_line(aes(color = Foreground)) + scale_color_brewer(palette = "Dark2") + scale_x_continuous(breaks=seq(0, 40, 5),name ="Number of Background species (nBg)")+ scale_y_continuous(breaks=seq(0, 21, 3),name ="Time of pseudogenization (MYA)")
i=i+2
}
